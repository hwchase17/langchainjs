...
test("Test ChatVectorDBQAChain from getDefaultTemplates to get default prompts", async () => {
  const defaultPrompts = ChatVectorDBQAChain.getDefaultTemplates();
  expect(defaultPrompts).toEqual(
    expect.objectContaining({
      questionGeneratorTemplate: expect.any(String),
      qaTemplate: expect.any(String),
    })
  );
